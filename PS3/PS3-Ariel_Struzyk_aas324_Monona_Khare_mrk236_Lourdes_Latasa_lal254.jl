...
md"""
### 
Ariel Struzyk (aas324), Monona Khare (mrk236), and Lourdes Latasa (lal254)
...
# ╔═╡ 7ab4d2d1-36d1-4c15-ab8a-a469a2da3fc6
 begin
	...
	# what is the index of the urea flux?
	idx_b₄1 = findall(x->x=="b4", rna)[1]
	c_vector1[idx_b₄1] = -1

	# compute the flux -
	result_case_2 = lib.flux(S,flux_bounds_array1,species_bounds_array1,c_vector1);
...
# check:
with_terminal() do
	ef1 = result_case_2.exit_flag
	sf1 = result_case_2.status_flag

	if (ef1 == 0.0 && sf1 == 5.0)
		println("Case 2: Optimal solution found. exit flag = $(ef1) and status flag = $(sf1)")
	else
		println("Case 2: Ooops! Check your problem setup. exit flag = $(ef1) and status flag = $(sf1)")
	end
end
...
let
	...
	# get flux values from the result -
	calculated_flux_array1 = result_case_2.calculated_flux_array

	# build flux table -
	flux_table1 = Array{Any,2}(undef,ℛ,4)

	# populate -
	for i ∈ 1:ℛ
		flux_table1[i,1] = i
		flux_table1[i,2] = rna[i]
		flux_table1[i,3] = calculated_flux_array1[i]*(1) # units: μmol/gDW-hr
		flux_table1[i,4] = expanded_reaction_array[i]
	end
	...
	with_terminal() do
		pretty_table(flux_table1; header=header_row, alignment=:l)
	end
end
